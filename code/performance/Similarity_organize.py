...
def all_paths_to_top(term, godag, silent=False):
...
def all_paths_to_top_wang(term, godag, optional_relationships, silent=False):
    # inputs: term_id and Go dag with 'relationship' as optional attributes
    """ Returns all possible paths to the root node"""
    if term not in godag:
        if silent:
...
def Semantic_Value(go_id, go, method, silent=False):
    ...
    if method == 'wang':
        ...
        all_all_paths = all_paths_to_top_wang(go_id, go, optional_relationships, silent=silent)
        S_values = list()
    ...
    #### Baseline: Almost Same as Wang. Only difference is the zero weight of root ancestor and consider regulated realtionships
        ...
        all_all_paths = all_paths_to_top(go_id, go, silent=silent)
        S_values = list()
        for index, path in enumerate(all_all_paths):
            S_values.append([])
            path.reverse()
...
def all_paths_to_bottom(term, godag, x, silent=False):
...
def Downward_Semantic_Value(go_id, go, x, silent=False):
    ...
    all_all_paths = all_paths_to_bottom(go_id, go, x, silent=silent)
    S_values = list()
    for index, path in enumerate(all_all_paths):
        S_values.append([])
        path.reverse()
...
def Similarity_of_Two_GOTerms(go_id1, go_id2, go, method, silent=False):
    if method == 'Baseline_LCA':
        lca = lowest_common_ancestor((go_id1, go_id2), go)
        sim_lca = Semantic_Value(lca, go, method, silent=silent)
        sim1 = Semantic_Value(go_id1, go, method, silent=silent)
        sim2 = Semantic_Value(go_id2, go, method, silent=silent) # sim1[1] and sim2[1] are the sums of all the nodes on the path with the max s-values. 
        ...
        if hcd != 0:
            ...
            sv_a = Downward_Semantic_Value(go_id1, go, x, silent=silent) # path to the specified depth
            sv_b = Downward_Semantic_Value(go_id2, go, y, silent=silent)
        ...
        sim_upper = Similarity_of_Two_GOTerms(go_id1, go_id2, go, 'Baseline_LCA', silent=silent)
        sim = (sim_down*0.5) + (sim_upper*0.5)
        #sim = (sim_down*0.3) + (sim_upper*0.7)
        return sim
    
    elif method == 'wang' or method == 'Baseline' or method == 'GOGO':
        sv_a = Semantic_Value(go_id1, go, method, silent=silent)
        sv_b = Semantic_Value(go_id2, go,  method, silent=silent)
        intersecting_terms = intersection(sv_a, sv_b)
        ...
        if hcd != 0:
            ...
            sv_a = Downward_Semantic_Value(go_id1, go, x, silent=silent)
            sv_b = Downward_Semantic_Value(go_id2, go, y, silent=silent)
            intersecting_terms = intersection(sv_a,sv_b)
            numerator = sum([x for t in intersecting_terms for x in t]) # Sum of common terms in all paths wrt to each of the 2 terms
            denominator = sum(x for y, x in sv_a) + sum(x for y, x in sv_b) #(where sv_a has 2 values for each term, second being the SV)
        ...
        sim_upper = Similarity_of_Two_GOTerms(go_id1, go_id2, go, 'Baseline', silent=silent)
        sim = (sim_down*0.5) + (sim_upper*0.5)
        #sim = (sim_down*0.3) + (sim_upper*0.7)
        return sim
...
# def Similarity_of_Set_of_GOTerms(set1, set2, go, method):
#     Sim1 = []
#     Sim2 = []
#     for idx, goterm in enumerate(set1):
#         Sim1.append([])
#         for goid in set2:
#             Sim1[idx].append((goterm, goid,(Similarity_of_Two_GOTerms(goterm, goid, go, method, silent=silent))))
#     for idx, goterm in enumerate(set2):
#         Sim2.append([])
#         for goid in set1:
#             Sim2[idx].append((goterm, goid,(Similarity_of_Two_GOTerms(goterm, goid, go, method, silent=silent))))
...
def Similarity_of_Set_of_GOTerms(set1, set2, go, method, silent=False):
    Sim1 = []
    Sim2 = []
    for idx, goterm in enumerate(set1):
        Sim1.append([])
    ...
    Sim1 = [i for i in Sim1 if len(i) > 0]
    Sim2 = [i for i in Sim2 if len(i) > 0]
    ...
    similarity = (sum(x[2] for x in sem1)+ sum(x[2] for x in sem2) )/(len(sem1) + len(sem2))
    return round(similarity, 3)
