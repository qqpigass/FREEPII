...
parser.add_argument('-root',        '--root',             default='/FREEPII_github', help='root',                          type=str)
...
parser.add_argument('-out_path',    '--output_path',      default='/FREEPII_output', help='output_path',                   type=str)
...
if '\\' in os.getcwd():
    args.root = ['/'.join(os.getcwd().split('\\')[:(i+1)]) for i,v in enumerate(os.getcwd().split('\\')) 
                 if v=='FREEPII_github'][0]

args.root = ['/'.join(os.getcwd().split('/')[:(i+1)]) for i,v in enumerate(os.getcwd().split('/')) if v=='FREEPII_github'][0]

if args.output_path=='/FREEPII_output':
    args.output_path = '/'.join([args.root + args.output_path, args.exp_name, args.exp_cond])


def process_complex(cur_species='Human', cur_exp_name='PXD002892', cur_exp_cond='SEC2-heavy', cur_root='/FREEPII_github'):
    ...
    if process_indicator:
        ...
        complex_df_raw = complex_df_raw[['ComplexName', 'subunits(UniProt IDs)', 'subunits(Gene name)', 'subunits(Entrez IDs)']].drop_duplicates()
        ...
        complex_df = pd.merge(complex_df, uniprot_df[['UniProt_entry', 'Gene_name', 'Gene_name_primary']], how='left', on='Gene_name').drop_duplicates()
        ...
        b = pd.melt(b, id_vars=['ComplexName', 'Gene_name', 'Gene_name_primary', 'Gene_entrez'], value_vars=['UniProt_entry_x', 'UniProt_entry_y'])
...
def process_epf(cur_species='Human', cur_exp_name='PXD002892', cur_exp_cond='SEC2-heavy', cur_root='/FREEPII_github', quant_type='iBAQ'):
...
def process_seq(cur_species='Human', cur_exp_name='PXD002892', cur_exp_cond='SEC2-heavy', cur_root='/FREEPII_github'):
    input_path = '/'.join([cur_root + '/input', cur_exp_name, cur_exp_cond])
    if not os.path.exists(input_path):
        os.makedirs(input_path)
...
def generate_split(cur_species='Human', cur_exp_name='PXD002892', cur_exp_cond='SEC2-heavy', cur_root='/FREEPII_github', 
    ...
    cur_com_dict['Gene_name'] = [sorted(set(cur_com_dict['Gene_name_A'][i] + cur_com_dict['Gene_name_B'][i]), key=lambda x: x.lower()) for i in range(cur_com_dict.shape[0])]
    cur_com_dict = dict(zip(cur_com_dict.index, cur_com_dict['Gene_name'].values))
    cur_com_N = pd.DataFrame({'ComplexName': cur_com_dict.keys(), 'Gene_name_N': [len(v) for k, v in cur_com_dict.items()]})
